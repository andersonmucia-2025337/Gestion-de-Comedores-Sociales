...
class ComedorService {
  ...
  async registrarUsuarioHibrido(datos: {
    ...
    let id_rol = 3; // Donante por defecto
    ...
    if (datos.rol === "admin" || datos.rol === "almacenista") {
      ...
      // Registrar Donante de forma transaccional directa
  ...
  // Corrección: Se agregó '| null' al tipo de retorno
  ...
  // --- REPORTES E INNOVACIÓN PARA EL ALMACENISTA ---
  ...
  // --- PROMESAS DE DONACIÓN (DONANTE ACTIVO) ---
  ...
  async procesarPromesa(idPromesa: number, idProducto: number, idPersonal: number, estado: "recibida" | "rechazada"): Promise<void> {
    await pool.query("CALL sp_procesarpromesadonacion(?, ?, ?, ?)", [idPromesa, idProducto, idPersonal, estado]);
  }
